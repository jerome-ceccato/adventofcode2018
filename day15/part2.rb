...
class Unit
  ...
  def initialize(position, type, ad)
    @type = type
    @position = position
    @hp = 200
    @damage = ad
...
class Game
  ...
  def initialize(filename, elf_ad)
    @world = []
    @units = []
    @uindex = {}
    @elf_ad = elf_ad
  ...
  def read_file(filename)
    ...
    File.foreach(filename) do |line|
      ...
      line.each_char.with_index do |c, x|
        if c == 'G'
          @units << Unit.new(Point.new(x, y), :goblin, 3)
          line[x] = '.'
        elsif c == 'E'
          @units << Unit.new(Point.new(x, y), :elf, @elf_ad)
          line[x] = '.'
        end
      end
      @world << line
  ...
  def result_if_flawless_victory
    elves = @units.count do |u|
      u.type == :elf
    end
    ...
    remaining_elves = @units.count do |u|
      u.type == :elf
    end

    if elves == remaining_elves
      result
    else
      -1
    end
...
def run_with_range(filename, i, j)
  target = i + (j - i) / 2
  result = Game.new(filename, target).result_if_flawless_victory
  if target == i
    if result >= 0
      result
    else
      Game.new(filename, i + 1).solve
    end
  elsif result >= 0
    run_with_range(filename, i, target)
  else
    run_with_range(filename, target, j)
  end
end

def main
  result = run_with_range("input", 4, 1000)
  puts "#{result}"
end
